...
(ED. NOTE FROM PAK: use "npx dotenv sequelize-cli" instead of "npm run", in the above)
...
heroku run npx sequelize-cli db:migrate (does not require starting w/"heroku run bash"?)
...
(PAK NOTE: also add in config vars for JWT_EXPIRES_IN and JWT_SECRET)
